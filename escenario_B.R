...
# Mientras el número de machos del banco de datos de la descendencia no sea
# igual a 3, sigue cruzan a los progenitores iniciales.
while ((sum(subset(descendencia_total, linea == "linea1")$sexo == "Macho") == 3 
      | sum(subset(descendencia_total, linea == "linea2")$sexo == "Macho") == 3 
      | sum(subset(descendencia_total, linea == "linea3")$sexo == "Macho") == 3)
      == FALSE){
...
for (z in 1:5000){
  for (j in 1:n){
    ...
    while ((sum(subset(descendencia_total, linea == "linea1")$sexo == "Macho") 
            == 3 | 
            sum(subset(descendencia_total, linea == "linea2")$sexo == "Macho") 
            == 3 | 
            sum(subset(descendencia_total, linea == "linea3")$sexo == "Macho") 
            == 3) == FALSE){
    ...
    parentales <- apareamiento_B("Escenario B", descendencia_total)
...
num_AA_mean_n50_esB <- round(apply(num_AA_mean, 2, mean))
num_Aa_mean_n50_esB <- round(apply(num_Aa_mean, 2, mean))
num_aa_mean_n50_esB <- round(apply(num_aa_mean, 2, mean))
